...
def create_planet(pos_x, pos_y):
    ...
    planet = pg.Rect(pos_x - (10*mass/2), pos_y - (10*mass/2), mass*10, mass*10)
...
#create controls
set_v0_to_0 = Toggle(screen, 925, 170, 50, 25)

#trail
show_trail = Toggle(screen, 925, 210, 50, 25)

#show center of mass
cm_value = Toggle(screen, 925, 250, 50,25)

#gravity
grav_const = TextBox(screen, 905, 310, 50, 50, fontSize=20)
grav_const.disable()
...
#velocity
t_const = Slider(screen, 975, 390, 200, 25, min=0.01, max = 0.1, step=0.01, initial=0.05)
t_const_display = TextBox(screen, 905, 380, 50, 50, fontSize=20)

#time step
t_step = Slider(screen, 975, 460, 200, 25, min=.1, max = 5.0, step = 0.25, initial = 1)
t_step_display = TextBox(screen, 905, 450, 50, 50, fontSize=20)

#mass
mass_slider = Slider(screen, 975, 530, 200, 25, min=.5, max = 5.0, step = .5, initial = 1)
mass_slider_display = TextBox(screen, 905, 520, 50, 50, fontSize=20)

#collision constant
collision_slider = Slider(screen, 975, 600, 200, 25, min=1, max=20, step=1, initial = 10)
collision_display = TextBox(screen, 905, 590, 50, 50, fontSize=20)

#preloaded systems
design_1 = Button(screen, 900, 650, 100, 50, text="Triangle", inactiveColour=(255, 255, 0), hoverColour=(255, 255, 0), fontSize=18, onClick=lambda: scalene())
design_2 = Button(screen, 1000, 650, 100, 50, text="Trapezoid", inactiveColour=(255, 0, 255), hoverColour=(255, 0, 255), fontSize=20, onClick=lambda: trapezoid())
design_3 = Button(screen, 1100, 650, 100, 50, text="Orbit", inactiveColour=(0, 255, 255), hoverColout=(0, 255, 255), fontSize=18, onClick=lambda: two_body_orbit())

#save and load designs
save_design = Button(screen, 900, 700, 300, 50, text="Save Orbit", inactiveColour=(200, 200, 255), hoverColour=(200, 200, 255), fontSize=18, onClick=lambda: save_orbit())
load_saved_design = Button(screen, 900, 750, 300, 50, text="Load Saved Orbit", inactiveColour=(200, 255, 200), hoverColour=(200, 255, 200), fontSize=18, onClick=lambda: load_saved_orbit())
frames = 0
...
while True:
    ...
    draw_text("Set v0 = 0", pg.font.SysFont("Arial", 20), (0, 0, 0), 1000, 170)
    draw_text("Show Planet Trail", pg.font.SysFont("Arial", 20), (0, 0, 0), 1000, 210)
    draw_text("Show System's CM", pg.font.SysFont("Arial", 20), (0, 0, 0), 1000, 250)
    draw_text("Gravitational Constant ↓ ", pg.font.SysFont("Arial", 20), (0, 0, 0), 970, 290)
    draw_text("Collision Constant ↓ ", pg.font.SysFont("Arial", 20), (0, 0, 0), 1000, 570)
    grav_const.setText(grav_slider.getValue())
    system.set_g_constant(grav_slider.getValue())
    collision_display.setText(collision_slider.getValue())
    system.set_collide_const(collision_slider.getValue())

    #stuff for time_step
    draw_text("Time Step ↓ ", pg.font.SysFont("Arial", 20), (0, 0, 0), 1025, 430)
    t_step_display.setText(round(t_step.getValue(), 2))
    system.set_time_step(round(t_step.getValue(), 2))

    #add mass for planets
    draw_text("Mass ↓", pg.font.SysFont("Arial", 20), (0, 0, 0), 1050, 500)
    ...
    if set_v0_to_0.value == False:
        draw_text("Yeet Constant ↓ ", pg.font.SysFont("Arial", 20), (0, 0, 0), 1000, 360)
